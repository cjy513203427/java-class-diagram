...
export async function activate(context: vscode.ExtensionContext) {
    ...
    const generateFolderDiagramCommand = vscode.commands.registerCommand(
        'javaClassDiagram.generateFolderDiagram',
        async (uri: vscode.Uri) => {
            try {
    ...
    const testLargeProjectCommand = vscode.commands.registerCommand(
        'javaClassDiagram.testLargeProject',
        async () => {
            try {
    ...
    const testSystemParsingCommand = vscode.commands.registerCommand(
        'javaClassDiagram.testSystemParsing',
        async () => {
            try {
                vscode.window.showInformationMessage('Testing system class parsing...');
                await testSystemClassParser();
...
async function navigateToClass(className: string, filePath: string): Promise<void> {
    try {
...
async function navigateToMethod(className: string, methodName: string, filePath: string): Promise<void> {
    try {
        const document = await vscode.workspace.openTextDocument(filePath);
        const editor = await vscode.window.showTextDocument(document, vscode.ViewColumn.Two);
...
async function showClassDetails(className: string, classStructures: any[]): Promise<void> {
